...
export default function PublicEventPage() {
  ...
  const [photo, setPhoto] = useState('');          // base64 uploaded photo
  const [composite, setComposite] = useState('');  // final composited image
  const [submitting, setSubmitting] = useState(false);
  const [result, setResult] = useState(null);      // { ticketCode, compositeImage }
  ...
  async function handleSubmit(e) {
    ...
    // If there's a frame image and user uploaded a photo, wait for composite
    const compositeImage = composite || '';

    const res = await fetch(`/api/public/${code}/rsvp`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...form, photo, compositeImage }),
    });
    ...
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div className="mb-8">
        <div className="flex items-start gap-3">
          <div className="flex-1">
            ...
            <p className="text-[#546048] mt-2 text-sm space-y-1">
              <span className="block">📅 {eventDate.toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}{' '}
                at {eventDate.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</span>
      ...
      <div className="bg-[#f2f1eb] border border-[#c0bfb9] rounded-2xl p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Photo upload (only shown if event has a frame) */}
          {event.frameImage && (
            <div>
          ...
          {/* Avatar compositor — runs when both frame + photo are ready */}
          ...
          >
            {submitting
              ? 'Registering…'
              : event.frameImage && photo && !composite
